...
After the index has been created, reads are then quantified using a minimum of 
30 Gibbs re-samples. The Gibbs re-sampled quants are imported into R using `edgeR::catchSalmon()`. The original counts from Salmon along with the overdispersion corrected counts from `catchSalmon()` are stored as separate 
